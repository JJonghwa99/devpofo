...
## 기술 스택
* **Backend**: Java 21, Spring Boot, Spring Web/WebFlux, Spring Data JPA, Spring Security / 추천 서버 : Python, Flask, NumPy, Pandas, Scikit-learn, SQLAlchemy, Psycopg2
...
* `RecommendController`와 `LectureService`에서는 사용자의 이수 현황과 졸업 요건을 바탕으로 들어야 할 강의를 추천하며, 특히 교양 과목은 다른 사용자들의 수강 데이터를 분석하여 '가장 많이 들은 순'으로 추천하는 데이터 기반 추천 로직을 적용하였으나 프로젝트 종료 이후
  개인적으로 `scikit-learn`을 활용하여 코사인 유사도 기반으로 사용자 간의 유사도를 분석하고, 가장 유사한 사용자들의 수강 데이터를 바탕으로 강의를 추천하는 로직을 적용.
